...
def create():
    ...
    phone_code = data.get('phone_code', '20') or '20'

    # Normalize phone using service logic
    phone = service.normalize_phone(whatsapp_raw, phone_code)
    lookup_key = phone.get("lookup_key")
    normalized_whatsapp = phone.get("normalized_whatsapp")
...
def update(traveler_id):
    ...
    if 'whatsapp_raw' in data or 'phone_code' in data:
        ...
        new_code = data.get('phone_code', '').strip() or traveler.phone_code or '20'
        service = UnifiedCRMService()
        normalized = service.normalize_phone(new_raw, new_code)
        lookup_key = normalized['lookup_key']
        normalized_whatsapp = normalized['normalized_whatsapp']
        ...
        traveler.integrated_whatsapp = normalized['normalized_whatsapp'] or traveler.integrated_whatsapp
        traveler.normalized_whatsapp = normalized['normalized_whatsapp'] or traveler.normalized_whatsapp
        traveler.phone_lookup_key = normalized['lookup_key'] or traveler.phone_lookup_key
