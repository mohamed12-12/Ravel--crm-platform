...
_OFF_SCRIPT_CLASSIFIER_SYSTEM_PROMPT = (
    ...
    "instead\" (target_hint=\"Thailand\"), and also \"I don't want this trip, show me "
    "something else\", \"can we pick a different trip\" (target_hint=\"\" -- no "
    "specific trip named). Do NOT use this for a question that merely mentions "
    "another trip (comparisons, curiosity) without asking to replace the current "
    "one. Set target_hint to the specific trip name/destination only if one was "
    "actually stated -- leave it as an empty string when the customer wants a "
    "different trip but names none, so the caller knows there is nothing to resolve "
